...
class NS_Block(nn.Module):
    def __init__(self, channels_in=32, n_head=4, d_k=8, d_v=8, radius=[3, 3, 3, 3], dilation=[1, 3, 5, 7]):
        ...
        self.output_Linear_MS = nn.Conv3d(980, channels_in, 1, bias=False)
        self.output_Linear_MT = nn.Conv3d(n_head * d_v, channels_in, 1, bias=False)
    ...
    def forward(self, first, x):
        ...
        M_S = torch.cat(M_A, dim=2).permute(0, 2, 1, 3, 4)
        M_T = torch.cat(M_T, dim=2).permute(0, 2, 1, 3, 4)

        out_cat = self.output_Linear_MT(M_T) * self.output_Linear_MS(M_S)
