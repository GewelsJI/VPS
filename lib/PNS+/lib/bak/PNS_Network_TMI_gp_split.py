...
class PNSNet(nn.Module):
    ...
    def forward(self, x):
        if len(x.shape) == 4:  # Pretrain
            ...
            first_high_feature, high_feature = torch.split(high_feature, [1, 5], dim=1)
            first_low_feature, low_feature = torch.split(low_feature, [1, 5], dim=1)
            ...
            print('high_feature_local', high_feature_local)
            print('high_feature_global', high_feature_global)
...
if __name__ == "__main__":
    #  File "/home/Code/video-polyp-segmentation/lib/benchmark/video/PNS-Net/lib/PNS_Network_TMI_gp_split.py", line 127
    #     print('high_feature_local', high_feature_local)
    ...
    model = PNSNet().cuda()
    model.load_backbone(
        torch.load('../snapshot/PNS_pretrain/epoch_101/PNS_Pretrain.pth', map_location=torch.device('cpu')), logging)
    model.cuda().train()
