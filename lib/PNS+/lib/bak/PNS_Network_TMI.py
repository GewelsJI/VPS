...
class PNSNet(nn.Module):
    ...
    def forward(self, x):
        if len(x.shape) == 4:  # Pretrain
            ...
            high_feature_global = high_feature

            high_feature_local = self.NSB_local(high_feature_local) + high_feature_local

            high_feature_global[:, 1:6, ...] = high_feature_local  # TODO: Better aggregation?
            high_feature_global = self.NSB_global(high_feature_global) + high_feature_global

            high_feature = high_feature + high_feature_global
            high_feature = high_feature.view(-1, *high_feature.shape[2:])  # torch.Size([5, 32, 16, 28])
